...
# ═══════════════════════════════════════════════════════════════════════════════
# MODULE 3: FORENSICS & STATE MACHINE
# ═══════════════════════════════════════════════════════════════════════════════
...
def build_state_machine(df):
    ...
    for i in range(len(ad_events)-1):
        ...
        if status == 'VERIFIED':
            ...
            if days <= 0:
                status = 'QUARANTINE_TIME'; flags.append("Negative Time Delta")
    ...
    with t3:
        ...
        with c2:
            if cum_drift:
                ...
                fig_d.update_layout(**_BL,title='Cumulative Physical vs Logged Mass Drift',yaxis=dict(title='FO_A − FO_L (MT)',**_AX),xaxis=dict(automargin=True, **_AX))
                fig_d.update_xaxes(tickangle=-45); st.plotly_chart(fig_d, use_container_width=True, config={'displayModeBar':False})
