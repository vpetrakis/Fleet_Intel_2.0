...
# ═══════════════════════════════════════════════════════════════════════════════
# TITAN CORE: v29.0 ENTERPRISE MONOLITH (With Sequence Smoothing)
# ═══════════════════════════════════════════════════════════════════════════════
try:
...
ICONS={"VERIFIED":_u('<svg viewBox="0 0 28 28" xmlns="http://www.w3.org/2000/svg"><circle cx="14" cy="14" r="12" fill="none" stroke="#00e0b0" stroke-width="1" opacity=".2"/><circle cx="14" cy="14" r="7.5" fill="#061a14" stroke="#00e0b0" stroke-width="1.2"/><polyline points="10,14.5 12.8,17 18,10.5" fill="none" stroke="#00e0b0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>'),"GHOST BUNKER":_u('<svg viewBox="0 0 28 28" xmlns="http://www.w3.org/2000/svg"><circle cx="14" cy="14" r="12" fill="none" stroke="#e63946" stroke-width="1" stroke-dasharray="4 3"/><circle cx="14" cy="14" r="7.5" fill="#1a0508" stroke="#e63946" stroke-width="1.2"/><g stroke="#e63946" stroke-width="2" stroke-linecap="round"><line x1="11" y1="11" x2="17" y2="17"/><line x1="17" y1="11" x2="11" y2="17"/></g></svg>'),"STAT OUTLIER":_u('<svg viewBox="0 0 28 28" xmlns="http://www.w3.org/2000/svg"><rect x="4" y="4" width="20" height="20" rx="5" fill="none" stroke="#7b68ee" stroke-width="1.2"/><circle cx="14" cy="14" r="4.5" fill="#0e0a1e" stroke="#7b68ee" stroke-width="1.2"/><circle cx="14" cy="14" r="1.8" fill="#7b68ee"/></svg>')}
...
# ═══════════════════════════════════════════════════════════════════════════════
# DATA EXTRACTION & SMOOTHING UTILS
# ═══════════════════════════════════════════════════════════════════════════════
def _sn(val):
...
req_cols = ['FO_A', 'FO_L', 'MGO_A', 'Bunk_FO', 'Bunk_MGO', 'Bunk_MELO', 'Bunk_HSCYLO', 'Bunk_LSCYLO', 'Bunk_GELO', 'Bunk_CYLO', 'MELO_R', 'HSCYLO_R', 'LSCYLO_R', 'GELO_R', 'CYLO_R', 'Speed', 'DistLeg', 'TotalDist', 'CargoQty', 'Voy', 'Port', 'AD', 'Date', 'Time']
for req in req_cols:
if req not in df.columns:
df[req] = np.nan if req in ['FO_A', 'FO_L', 'MGO_A', 'MELO_R', 'HSCYLO_R', 'LSCYLO_R', 'GELO_R', 'CYLO_R'] else (0.0 if req not in ['Voy', 'Port', 'AD', 'Date', 'Time'] else '')
...
# --- SEQUENCE SMOOTHER (Fixes the Quarantine Bugs) ---
ad_events = df[df['AD'].isin(['A', 'D'])].copy()
if len(ad_events) < 2: return pd.DataFrame(), vname, {}, []

# Merge consecutive identical states (e.g. D -> D becomes just D)
...
if days <= 0: days = 0.04 # Fallback to ~1 hour to prevent division crashes

start_rob, end_rob = _sn(r1.get('FO_A')), _sn(r2.get('FO_A'))
if pd.isna(start_rob) or pd.isna(end_rob):
status = 'QUARANTINE_ROB'; flags.append("Missing Tank Sounding")
...
phys_burn, log_burn, drift, daily_burn, dqi = np.nan, np.nan, np.nan, np.nan, 0
if status == 'VERIFIED':
...
if phase == 'PORT' and phys_burn < -5.0:
status = 'GHOST BUNKER'; flags.append("Missing Port Bunker")
elif phase == 'SEA' and phys_burn < -2.0:
status = 'GHOST BUNKER'; flags.append("Negative Sea Burn")
...
# --- AI PHYSICS ENGINE (Duration Weighted) ---
ai_msg = "AI Optimized."
...
if HAS_ML and not trip_df.empty:
try:
...
var_model.fit(X_train, np.abs(y_train - model.predict(X_train)), sample_weight=weights)

preds, stoch_margin = model.predict(X_train), np.maximum(var_model.predict(X_train) * 1.645, 0.5)
...
else: ai_msg = f"AI Offline: Insufficient Sea Legs ({sea_mask.sum()})."
except Exception as e: ai_msg = f"AI Exception: {str(e)}"
else: ai_msg = "AI Offline: Libraries not found."

quar = len(trip_df[trip_df['Status'].str.contains('QUARANTINE')])
val_sea = trip_df[(trip_df['Phase'] == 'SEA') & (trip_df['Status'] == 'VERIFIED')]
avg_sea = val_sea['Phys_Burn'].sum() / val_sea['Days'].sum() if val_sea['Days'].sum() > 0 else 0.0

summary = {
'vname': vname,
'integrity': round((len(trip_df) - quar) / len(trip_df) * 100, 1) if not trip_df.empty else 0,
...
return trip_df, summary, cum_drift
...
try:
df, sum_data, cum_drift = process_pipeline(f)
if df.empty: st.warning(f"No valid events extracted from {f.name}. Check template schema."); continue
fleet_results.append({'name': sum_data['vname'], 'summary': sum_data, 'df': df})

ic = SC['VERIFIED'] if sum_data['integrity'] >= 80 else (SC['STAT OUTLIER'] if sum_data['integrity'] >= 50 else SC['GHOST BUNKER'])
...
with c1:
fig_l = go.Figure()
if df['MELO_L'].sum() > 0: fig_l.add_trace(go.Bar(x=df['Timeline'], y=df['MELO_L'], name='MELO', marker_color='rgba(0,224,176,0.12)', marker_line_color='#00e0b0', marker_line_width=1.3))
if df['Total_CYLO'].sum() > 0: fig_l.add_trace(go.Bar(x=df['Timeline'], y=df['Total_CYLO'], name='CYLO (All)', marker_color='rgba(123,104,238,0.12)', marker_line_color='#7b68ee', marker_line_width=1.3))
if df['GELO_L'].sum() > 0: fig_l.add_trace(go.Bar(x=df['Timeline'], y=df['GELO_L'], name='GELO', marker_color='rgba(201,168,76,0.12)', marker_line_color='#c9a84c', marker_line_width=1.3))
fig_l.update_layout(**_BL, title='Lubricant Consumption (Liters)', barmode='group', showlegend=True, yaxis=dict(title='L', **_AX), xaxis=dict(automargin=True, **_AX)); fig_l.update_xaxes(tickangle=-45); st.plotly_chart(fig_l, use_container_width=True, config={'displayModeBar':False})
with c2:
if cum_drift:
